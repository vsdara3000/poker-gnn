...
**mccfr (tabular) vs. deep_cfr on HULHE, first real comparison** -- ran both via the new CLI, seed 0, default hyperparameters, `baseline_eval` every checkpoint (200 hands/matchup):

- `mccfr`, 150,000 iterations (~2.4ms/iter, no network -- 6 checkpoints of 25k each): `vs_fold` stayed solidly positive (+1.1 to +1.5 as P0) the whole run, but `vs_call` stayed *negative* the entire time (-0.19 to -0.64 as P0, no checkpoint ever positive) and `vs_random` as P0 was flat-to-negative (+1.6 at 75k down to **-0.91 at 150k**, no visible upward trend at all across 6x the iterations).
- `deep_cfr` (external-sampling, GNN + strategy network), 1200 iterations (~280ms/iter, 6 checkpoints of 200 each): noisy but already stronger on the metric that matters most -- `vs_random` as P0 ranged +2.4 to +7.2 across every checkpoint (never negative, unlike mccfr's -0.91), and `vs_call` was positive at half the checkpoints (+1.15, +1.15) vs. mccfr's *zero* positive checkpoints in 150k iterations.
- So deep_cfr at ~125x fewer iterations and roughly comparable wall-clock time (1200 x 0.28s ≈ 340s vs. mccfr's 150,000 x 0.0024s ≈ 360s) is already ahead on the baseline-eval numbers that matter, not just noisier-but-similar.
- Investigated why mccfr looks flat rather than improving: pulled `mccfr`'s trained `average_strategy()` dict after the 150k-iteration run (12,404,351 distinct infosets -- external sampling still branches over every action on the traverser's own turns, so the dict grows fast even though the tree can't be exhausted) and checked how often a *fresh random-deal* hand's decision points actually land in it: **only 48.3%** (4110/8510 decision points sampled). `baseline_eval`'s `make_strategy_policy` falls back to uniform random for anything not in the dict (see its docstring), so roughly half of every mccfr evaluation hand is unlearned random play -- that's the likely reason its numbers don't trend anywhere across 6x the iterations. `deep_cfr`'s `policy()` has no such gap (network fallback covers 100% of states), so its baseline-eval numbers are actually measuring what the solver learned.
- Takeaway: `baseline_eval` on a raw tabular dict (no generalizing fallback) is not a fair or informative progress metric at HULHE's scale, independent of how many iterations you run it for -- a like-for-like comparison would need to pair `mccfr` with some generalizing average-strategy estimate (e.g. a strategy network trained the same way `deep_cfr`'s is, but fed from `mccfr`'s sampled strategies instead of a GNN's), which doesn't exist yet. Until then, `mccfr`'s only real purpose here is as a much-cheaper-per-iteration correctness/regret sanity check, not a baseline-eval number to compare directly against `deep_cfr`.
- Confirmed separately (different session, "bigger" deep_cfr config -- hidden_dim=64/buffer=20000/train_steps=8 -- instead of the default above): same conclusion holds. `mccfr` at 130,000 iterations (~1100s, 10.67M infosets seen) still lost to always-call with both seats (P0/P1 = -0.034/-0.184) and was roughly break-even vs. random as P0 (-0.021); `deep_cfr` at 5,000 iterations in the same ~1070s wall-clock budget (490,578 infosets seen) beat always-call with both seats (+0.638/+0.036) and crushed random (+2.942/+1.552). Not a new finding, just extra confidence the pattern isn't specific to default hyperparameters.

Infrastructure done since, without a long run (from a "what can we do that doesn't need long runs" discussion):
